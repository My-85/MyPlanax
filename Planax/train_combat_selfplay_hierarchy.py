...
class ActorCriticRNN(nn.Module):
    ...
    def __call__(self, hidden, x):
        ...
        # RNN 层的关键特性
        # 输入/输出：
        # 输入：来自 Dense 层的特征向量 + 重置标志 dones
        # 输出：时序增强的特征向量（传递给后续 Dense 层）
        # 实现方式：
        # 通过 ScannedRNN 类实现（基于 nn.scan 的高效循环）
        # 使用 GRU 单元（在 Planax baseline 中）
        # 支持隐藏状态重置（当 dones=True 时重置状态）
        # 作用：
        # 捕捉时序依赖（如飞机姿态变化趋势）
        # 维持跨时间步的记忆（如追踪敌机位置变化）
        # 处理变长序列（适应不同长度的 episode）
        ################################################################
        actor_mean = nn.Dense(
            ...
            def _update_epoch(update_state, unused):
                def _update_minbatch(train_state, batch_info):
                    ...
                    def _loss_fn(params, init_hstate, traj_batch, gae, targets):
                        ...
                        entropy = pi[0].entropy().mean() + pi[1].entropy().mean() + pi[2].entropy().mean()
