...
config = {
    ...
    "SEED": 42,
    "FOR_LOOP_EPOCHS": 60, # LSTM 版引入 for 循环（FOR_LOOP_EPOCHS=5），每次迭代加载上一个 checkpoint，继续训练（增量训练）。
    "LR": 3e-4,
    "NUM_ENVS": 1000,
    "NUM_ACTORS": 1,
    "NUM_STEPS": 1000,
    ...
    # "LOADDIR": "/home/dqy/NeuralPlanex/Planax_lczh/Planax_lczh/results/heading_pitch_V_discrete_lstm_2025-08-14-17-41/checkpoints/checkpoint_epoch_50" 
}
...
for i in range(config["FOR_LOOP_EPOCHS"]): # LSTM 版引入 for 循环（FOR_LOOP_EPOCHS=5），每次迭代加载上一个 checkpoint，继续训练（增量训练）。
