...
class Transition(NamedTuple):
    ...
    info: jnp.ndarray
    action_dist: jnp.ndarray  # 添加动作分布信息
        ...
        def _update_step(update_runner_state, unused):
            ...
            def _env_step(runner_state, unused):
                ...
                transition = Transition(
                    last_done, action, value, reward, log_prob, last_obs, last_global_obs, ~(last_done & done), info, pi
                )
            ...
            if config.get("DEBUG"):
                def callback(metric):
                    ...
                    writer.add_scalar('debug/adv_max', advantages.max(), env_steps)
                    writer.add_scalar('debug/adv_min', advantages.min(), env_steps)
                    writer.add_scalar('debug/adv_mean', advantages.mean(), env_steps)
                    ...
                    # 添加策略分布可视化指标
                    # 计算每个智能体的动作分布熵值
                    action_entropies = jnp.array([p.entropy() for p in traj_batch.action_dist])
                    # 计算所有智能体的平均熵值
                    mean_entropy = jnp.mean(action_entropies)
                    # 计算熵值的标准差，用于判断不同智能体的策略是否一致
                    entropy_std = jnp.std(action_entropies)
                    # 记录最大熵值，用于判断是否有智能体的策略过于均匀
                    max_entropy = jnp.max(action_entropies)
                    
                    # 记录log probability的统计信息
                    log_prob_mean = jnp.mean(traj_batch.log_prob)
                    log_prob_std = jnp.std(traj_batch.log_prob)
                    log_prob_min = jnp.min(traj_batch.log_prob)
                    log_prob_max = jnp.max(traj_batch.log_prob)
                    
                    # 将策略分布指标写入TensorBoard
                    writer.add_scalar('eval/mean_entropy', mean_entropy, env_steps)
                    writer.add_scalar('eval/entropy_std', entropy_std, env_steps)
                    writer.add_scalar('eval/max_entropy', max_entropy, env_steps)
                    writer.add_scalar('eval/log_prob_mean', log_prob_mean, env_steps)
                    writer.add_scalar('eval/log_prob_std', log_prob_std, env_steps)
                    writer.add_scalar('eval/log_prob_min', log_prob_min, env_steps)
                    writer.add_scalar('eval/log_prob_max', log_prob_max, env_steps)
                    
                    # 将策略指标添加到metric字典中 - 与loss相同的处理方式
                    if "loss" in metric:
                        # 将策略指标添加到loss字典中，这样会被wandb自动记录
                        metric["loss"]["mean_entropy"] = mean_entropy
                        metric["loss"]["entropy_std"] = entropy_std
                        metric["loss"]["max_entropy"] = max_entropy
                        metric["loss"]["log_prob_mean"] = log_prob_mean
                        metric["loss"]["log_prob_std"] = log_prob_std
                        metric["loss"]["log_prob_min"] = log_prob_min
                        metric["loss"]["log_prob_max"] = log_prob_max
                    
                    wandb.log(metric)
                    wandb.log({"adv_std": advantages.std(), "adv_max": advantages.max(), "adv_min": advantages.min(), "adv_mean": advantages.mean()})
                    wandb.log({'log_prob_mean': log_prob_mean, 'log_prob_std': log_prob_std, 'log_prob_min': log_prob_min, 'log_prob_max': log_prob_max})
                                       
                    # 同时更新打印的内容，使用更多格式化输出，确保在wandb控制台中可见
                    ...
                    print("Policy: MeanEntropy={:.3f} LogProbMean={:.3f} LogProbMin={:.3f} LogProbMax={:.3f}".format(
                        mean_entropy,
                        log_prob_mean,
                        log_prob_min,
                        log_prob_max
