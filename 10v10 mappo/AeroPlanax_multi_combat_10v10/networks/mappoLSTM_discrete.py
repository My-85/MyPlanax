...
from networks.scannedLSTM import ScannedLSTM
...
class ActorLSTMOldPattern(nn.Module):
    action_dim: Sequence[int]
    config: Dict
...
def init_network(config : Dict[str, Any], discrete_action_dims : List[int] = MAPPO_DISCRETE_DEFAULT_DIMS, old_model=False) -> Tuple[Tuple[nn.Module, nn.Module], Tuple[TrainState, TrainState], int]:
    ...
    # Replace GRU_HIDDEN_DIM with LSTM_HIDDEN_DIM if not already defined
    if "LSTM_HIDDEN_DIM" not in config:
        if "GRU_HIDDEN_DIM" in config:
            config["LSTM_HIDDEN_DIM"] = config["GRU_HIDDEN_DIM"]
        else:
            config["LSTM_HIDDEN_DIM"] = 256  # Default value if neither is defined
    ...
    if old_model:
        actor_network = ActorLSTMOldPattern(discrete_action_dims, config=config)
    else:
        actor_network = ActorLSTM(discrete_action_dims, config=config)

    critic_network = CriticLSTM(config=config)
    ...
    ac_init_hstate = ScannedLSTM.initialize_carry(config["NUM_ACTORS"] * config["NUM_ENVS"], config["LSTM_HIDDEN_DIM"])
    ...
    cr_init_hstate = ScannedLSTM.initialize_carry(config["NUM_ACTORS"] * config["NUM_ENVS"], config["LSTM_HIDDEN_DIM"])
