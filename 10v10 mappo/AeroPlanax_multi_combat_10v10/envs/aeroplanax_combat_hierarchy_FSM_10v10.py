...
from .utils.utils import  wedge_formation, line_formation, diamond_formation, enforce_safe_distance, get_AO_TA_R, wrap_PI, wrap_halfPI, get_AO_TA_R_pitch_yaw
...
CURRICULUM_MANEUVER_MAX = 0.6        # 敌方最大机动性能倍数
...
def custom_event_driven_reward_fn(
    ...
    reward = jnp.select(
        ...
        [complete_victory_reward, 
         great_victory_reward + ally_alive_count * extra_reward, 
         normal_victory_reward + (ally_alive_count - enemy_alive_count) * extra_reward, 
         narrow_victory_reward + (ally_total_blood - enemy_total_blood)/ally_alive_count, 
         fail_reward],
        default=0.0
    )
...
class AeroPlanaxHierarchicalCombatEnv(AeroPlanaxEnv[HierarchicalCombatTaskState, HierarchicalCombatTaskParams]):
    def __init__(self, env_params: Optional[HierarchicalCombatTaskParams] = None):
        ...
        self.reward_functions = [
            ...
            functools.partial(tactical_position_reward_fn, 
                              ...
                              num_allies=env_params.num_allies, 
                              num_enemies=env_params.num_enemies),
            functools.partial(crash_penalty_fn_new, penalty_value=-1.0),  # 降低惩罚从-500到-100
            functools.partial(enemy_kill_reward_fn,  # 新增击落奖励函数
                              kill_reward=100.0,  # 降低奖励从300到50
                              num_allies=env_params.num_allies,
                              num_enemies=env_params.num_enemies),
            functools.partial(teammate_death_penalty_fn,  # 新增队友死亡惩罚函数
                              death_penalty=-100.0,  # 降低惩罚从-50到-20
                              num_allies=env_params.num_allies,
                              num_enemies=env_params.num_enemies),
            functools.partial(custom_event_driven_reward_fn,
                              ...
                              complete_victory_reward=150.0),  # 降低奖励从500到100
            # functools.partial(event_driven_reward_fn, 
            #                   fail_reward=-100.0,  # 降低惩罚从-500到-100
            #                   success_reward=100.0),  # 降低奖励从500到100
        ]
        self.is_potential = [False,False,False,False,True,False,False,True]
        ...
        self.norm_delta_pitch = jnp.linspace(-jnp.pi/4, jnp.pi/4, 10)  # 30 steps from -30° to 30°
        self.norm_delta_heading = jnp.linspace(-jnp.pi/6, jnp.pi/6, 10)  # 30 steps from -30° to 30°
        self.norm_delta_velocity = jnp.linspace(-100, 100, 10)  # 30 steps from -100 to 100
        ...
        if not self.use_baseline:
            ...
            def single_enemy_fsm_logic(enm_local_idx, current_fsm_state, enm_key):
                ...
                def attack_action():
                    ...
                    # 根据课程学习进度调整敌方机动性能
                    max_pitch_change = jnp.pi/4 * maneuver_factor
                    max_heading_change = jnp.pi/6 * maneuver_factor
                    max_vt_change = 50 * maneuver_factor
            ...
            # 计算目标状态用于控制器
            target_pitch = wrap_halfPI(init_state.plane_state.pitch + delta_pitch)
            target_heading = wrap_PI(init_state.plane_state.yaw + delta_heading)
            target_vt = init_state.plane_state.vt + delta_vt
    ...
    def _observe_features(self, state: HierarchicalCombatTaskState, i: int, j_idx: int):
        ...
        # 计算机头方向和飞机连线之间的夹角
        ego_heading = state.plane_state.yaw[i]
        ego_pitch = state.plane_state.pitch[i]
        enm_heading = state.plane_state.yaw[j_idx]
        enm_pitch = state.plane_state.pitch[j_idx]
        
        # 计算目标方位角和俯仰角
        target_heading = jnp.arctan2(enm_feature[1] - ego_feature[1], enm_feature[0] - ego_feature[0])
        target_pitch = jnp.arctan2(enm_feature[2] - ego_feature[2], 
                                  jnp.sqrt((enm_feature[0] - ego_feature[0])**2 + 
                                         (enm_feature[1] - ego_feature[1])**2))
        
        # 计算己方和敌方飞机机头与连线的夹角差
        ego_delta_heading = jnp.abs(ego_heading - target_heading)
        ego_delta_heading = jnp.minimum(ego_delta_heading, 2*jnp.pi - ego_delta_heading) / jnp.pi  # 归一化到[0,1]
        
        enm_delta_heading = jnp.abs(enm_heading - target_heading + jnp.pi) # 加pi是因为敌方视角下连线方向相反
        enm_delta_heading = jnp.minimum(enm_delta_heading, 2*jnp.pi - enm_delta_heading) / jnp.pi  # 归一化到[0,1]
        
        ego_delta_pitch = jnp.abs(ego_pitch - target_pitch) / jnp.pi  # 归一化到[0,1]
        enm_delta_pitch = jnp.abs(enm_pitch - (-target_pitch)) / jnp.pi  # 归一化到[0,1], 加负号因为敌方视角下俯仰角相反
        ...
        # 归一化其他飞机血量
        norm_blood = state.plane_state.blood[j_idx] / 100
        
        features = jnp.hstack((norm_delta_vt, norm_delta_altitude, norm_AO, norm_TA, norm_distance, side_flag, ego_delta_heading, ego_delta_pitch, enm_delta_heading, enm_delta_pitch, norm_blood))
        return features
    ...
    def _get_own_features(self, state: HierarchicalCombatTaskState, i: int):
        ...
        yaw_sin = jnp.sin(yaw)
        yaw_cos = jnp.cos(yaw)
        blood = state.plane_state.blood[i] / 100
        empty_features = jnp.zeros(shape=(self.own_features,))
        features = jnp.hstack((norm_altitude, norm_vel_x, norm_vel_y, norm_vel_z, norm_vt, roll_sin, roll_cos, pitch_sin, pitch_cos, yaw_sin, yaw_cos, blood))
        return jax.lax.cond(
    ...
    def _generate_formation(
        ...
        # ))
        formation_positions = jnp.vstack((ally_positions+ally_center, enemy_positions+enemy_center))
