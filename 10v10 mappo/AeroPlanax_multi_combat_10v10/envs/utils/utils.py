...
def get_AO_TA_R(ego_feature, enm_feature):
    ...
    # Add safety checks for very small values
    R = jnp.maximum(R, 1e-6)  # Ensure R is not too small
    ego_v = jnp.maximum(ego_v, 1e-6)  # Ensure velocity is not too small
    enm_v = jnp.maximum(enm_v, 1e-6)  # Ensure velocity is not too small

    # Calculate projections with safety checks
    proj_dist_ego = delta_x * ego_vx + delta_y * ego_vy + delta_z * ego_vz
    proj_dist_enm = delta_x * enm_vx + delta_y * enm_vy + delta_z * enm_vz

    # Calculate angles with improved numerical stability
    denominator_ego = R * ego_v
    denominator_enm = R * enm_v

    # Clip the ratio to prevent numerical instability
    ratio_ego = jnp.clip(proj_dist_ego / denominator_ego, -0.9999, 0.9999)
    ratio_enm = jnp.clip(proj_dist_enm / denominator_enm, -0.9999, 0.9999)

    # Calculate angles
    ego_AO = jnp.arccos(ratio_ego)
    ego_TA = jnp.arccos(ratio_enm)

    # Handle edge cases
    ego_AO = jnp.where(jnp.isnan(ego_AO), jnp.pi/2, ego_AO)
    ego_TA = jnp.where(jnp.isnan(ego_TA), jnp.pi/2, ego_TA)

    # Calculate side flag with safety check
    cross_product = jnp.cross(jnp.hstack((ego_vx, ego_vy)), jnp.hstack((delta_x, delta_y)))
    side_flag = jnp.sign(cross_product)
    side_flag = jnp.where(jnp.isnan(side_flag), 0.0, side_flag)
...
def line_formation(num_agents, spacing):
    ...
    # 计算需要的层数
    num_layers = (num_agents + agents_per_layer - 1) // agents_per_layer
    
    def layer_loop(i, carry):
        positions, count = carry
        
        # 计算当前层的飞机数
        agents_this_layer = jnp.minimum(agents_per_layer, num_agents - i * agents_per_layer)
        
        # 计算当前层的起始x位置
        start_x = -(agents_this_layer - 1) * spacing / 2
        
        # 计算当前层的z位置
        z_pos = (i - (num_layers - 1) / 2) * spacing / 2 + 10000
        
        # 为当前层的每架飞机设置位置
        def agent_loop(j, positions):
            agent_idx = i * agents_per_layer + j
            x_pos = start_x + j * spacing
            positions = positions.at[agent_idx, 2].set(z_pos)      # down
            positions = positions.at[agent_idx, 1].set(x_pos)      # east
            return positions
            
        positions = jax.lax.fori_loop(0, agents_this_layer, agent_loop, positions)
        return positions, count + agents_this_layer
    
    positions, _ = jax.lax.fori_loop(0, num_layers, layer_loop, (positions, 0))
    return positions
