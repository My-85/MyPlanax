...
print(f'combat_hierarchy policy: load heading baseline from {os.path.join(os.getcwd(),"/home/lczh/Git Project/AeroPlanax-dev-tmp0429_lxy_reform/envs/models/baseline")}')
config = {
    "SEED": 42,
    "LR": 3e-4,
    "NUM_ENVS": 1,
    ...
    "ACTIVATION": "relu",
    "ANNEAL_LR": False,
    "LOADDIR": os.path.join(os.getcwd(),"/home/lczh/Git Project/AeroPlanax-dev-tmp0429_lxy_reform/envs/models/baseline/Heading_altitude")
}
...
class AeroPlanaxHierarchicalCombatEnv(AeroPlanaxEnv[HierarchicalCombatTaskState, HierarchicalCombatTaskParams]):
    def __init__(self, env_params: Optional[HierarchicalCombatTaskParams] = None):
        ...
        # self.norm_delta_pitch = jnp.linspace(-jnp.pi/6, jnp.pi/6, 10)  # 10 steps from -30° to 30°
        self.norm_delta_heading = jnp.linspace(-jnp.pi/6, jnp.pi/6, 10)  # 10 steps from -30° to 30°
        self.norm_delta_velocity = jnp.linspace(-0.15, 0.15, 10)  # 10 steps from -100 to 100
        self.norm_delta_altitude = jnp.linspace(-0.05, 0.05, 10)  # 10 steps from -100 to 100
